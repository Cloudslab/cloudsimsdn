...
package org.cloudbus.cloudsim.sdn.vmallocation.priority;
...
import org.cloudbus.cloudsim.sdn.PhysicalTopology;
import org.cloudbus.cloudsim.sdn.PhysicalTopology.NodeType;
...
public class VmAllocationPolicyPriorityFirst extends VmAllocationPolicyGroupConnectedFirst {
	...
	private Map<Node,HostGroup> getHostGroupMap() {
		...
		for(Node e: edges) {
			...
			hg.hosts = new ArrayList<>();
			for (Node node : topology.getConnectedNodesLow(e)) {
				hg.hosts.add((SDNHost)node);
			}

...
public class OverbookingVmAllocationPolicyConsolidateCorrelated extends OverbookingVmAllocationPolicy implements VmAllocationInGroup {
	public OverbookingVmAllocationPolicyConsolidateCorrelated(
	...
	public boolean allocateHostForVmInGroup(Vm vm, VmGroup vmGroup) {
		...
		if(correlatedHosts.size() == 0) {
			...
			if(allocateHostForVm(vm, hostSelectionPolicy.selectHostForVm((SDNVm)vm, correlatedHosts)) == true) {
				return true;
			}
			else {
